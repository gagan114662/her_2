# Revenue Outreach Log

This log tracks live revenue actions for iPOP and DesignGaga offers.

## Live Offers

- iPOP Automation Diagnostic: https://gagan114662.github.io/her_2/ipop.html
- iPOP Listing Prep Agent Sprint: https://gagan114662.github.io/her_2/listing-prep-agent-sprint.html
- iPOP real-domain paid work page: https://ipop.ai/#work
- iPOP Agent Trial Sprint: https://buy.stripe.com/eVq9AUfbN0Q72HpaF21kA07
- DesignGaga Home Staging Quick Consult: https://gagan114662.github.io/her_2/designgaga-consult.html

## Stripe Status

- 2026-05-15: Stripe live mode connected.
- 2026-05-15: Balance checked after outreach batches: 0 available, 0 pending, no payment intents.
- 2026-05-15: Gmail reply/bounce check found one hard bounce and no buyer replies.
- 2026-05-15: Created iPOP Listing Prep Agent Sprint live Stripe link for CAD 49: https://buy.stripe.com/cNicN64x9fL1a9RbJ61kA06
- 2026-05-15: Added paid agent-work offers to ipop.ai and created a USD 19 Agent Trial Sprint checkout: https://buy.stripe.com/eVq9AUfbN0Q72HpaF21kA07

## Sent Outreach

### 2026-05-15 iPOP Automation Diagnostic

- ravi@jainimmigrationlaw.com — Quick $49 website/automation diagnostic?
- prashant@rapida.ai — Small $49 automation diagnostic for Rapida?
- support@swasacceleratorr.com — Could build a $49 SWAS automation diagnostic
- chaitanya@getcreatr.com — Quick $49 automation diagnostic?
- nicole@getleo.com — Small website/automation diagnostic for real estate ops?
- harish.h@razorpay.com — Small checkout/payment automation diagnostic
- bob.pardo@pardo.space — Quick $49 automation/project diagnostic
- kirk@graphlit.com — Small agent implementation diagnostic for Graphlit users
- hello@returnmytime.com — Tiny implementation diagnostic for automation leads
- nexustrade@starks-technology.com — Quick automation diagnostic for trading workflow ops
- fatima@okara.ai — Small implementation diagnostic for AI marketing workflows
- adi@newsletter.agentmail.to — Small diagnostic for agent/workflow implementation gaps — bounced, domain not found

### 2026-05-15 DesignGaga Staging Consult

- tk@getleo.com — Quick $49 staging consult for listings?
- linavelascolozano@housesigma.com — Small DesignGaga staging consult for buyers/sellers
- ssukhjbkrealty@gmail.com — Quick $49 staging consult for listings
- johannes@golfiteam.com — Small staging consult for Golfi listings
- sesh@effortlessmortgage.ca — Small staging consult for mortgage/refi clients
- marketing@exclusive-links.com — Quick staging/design consult for property listings
- hello@dancooper.com — Quick $49 staging consult for listings — superseded by iPOP correction
- kelsey@dancooper.com — Quick $49 staging consult for listings — superseded by iPOP correction
- nancy@nancyshea.ca — Quick $49 staging consult for listings — superseded by iPOP correction
- sarah@sarahlogue.ca — Quick $49 staging consult for listings — superseded by iPOP correction
- lisa@lisaweberteam.ca — Quick $49 staging consult for listings — superseded by iPOP correction
- homes@alexandramaurice.ca — Quick $49 staging consult for listings — superseded by iPOP correction
- info@classicrealtygroup.ca — Quick $49 staging consult for listings — bounced, address not found

### 2026-05-15 iPOP Listing Prep Agent Sprint

- hello@dancooper.com — Correction: agent-delivered listing prep sprint
- kelsey@dancooper.com — Correction: agent-delivered listing prep sprint
- nancy@nancyshea.ca — Correction: agent-delivered listing prep sprint
- sarah@sarahlogue.ca — Correction: agent-delivered listing prep sprint
- lisa@lisaweberteam.ca — Correction: agent-delivered listing prep sprint
- homes@alexandramaurice.ca — Correction: agent-delivered listing prep sprint
- info@classicrealtygroup.ca — Correction: agent-delivered listing prep sprint — bounced, address not found

### 2026-05-15 iPOP Agent Trial Sprint

- prashant@rapida.ai — Tiny $19 agent-work trial is live
- chaitanya@getcreatr.com — Tiny $19 agent-work trial is live
- harish.h@razorpay.com — Tiny $19 agent-work trial is live
- bob.pardo@pardo.space — Tiny $19 agent-work trial is live
- kirk@graphlit.com — Tiny $19 agent-work trial is live
- hello@returnmytime.com — Tiny $19 agent-work trial is live
- nexustrade@starks-technology.com — Tiny $19 agent-work trial is live
- fatima@okara.ai — Tiny $19 agent-work trial is live

## Known Funnel Issues

- designgaga.ca currently has no public DNS answers and CIRA WHOIS returns Not found.
- WordPress/WPWriter has no connected DesignGaga site, so current public landing is GitHub Pages.
- adi@newsletter.agentmail.to bounced because the domain newsletter.agentmail.to could not be found.
- info@classicrealtygroup.ca bounced because the address could not be found or is unable to receive mail.
- Future revenue outreach should prefer iPOP/Samantha delivery-machine offers over offers that imply Gagan personally performs the service work.
- ipop.ai is the preferred public URL for new outreach because it is the owned domain and now links directly to paid Stripe agent-work offers.
